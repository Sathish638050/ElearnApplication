...
namespace ELearnApplication.Models
{
    public class Course : ICourse<Course>
    {
        ...
        public async void EditCourse(int id, Course c)
        {
            ...
            {
                StringContent content = new StringContent(JsonConvert.SerializeObject(c), Encoding.UTF8, "application/json");
                using (var response = await httpClient.PutAsync("https://localhost:44303/api/Staff/EditCourse?id=" + id, content))
                {
        ...
        public async Task<List<Course>> GetAllCourse()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:44359/api/Student/GetAllCourse"))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<List<Course>> getCourseByUser(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:44303/api/Staff/GetCourseByUser?id=" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                    lc = JsonConvert.DeserializeObject<List<Course>>(apiResponse);
                }
        ...
        public async Task<Course> GetDetailById(int id)
        {
            ...
            using(var httpClient = new HttpClient())
            {
                using(var response = await httpClient.GetAsync("https://localhost:44359/api/Student/GetCourseById?id=" + id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<List<Course>> GetEnrollCourses(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://localhost:44359/api/Student/GetEnrollCourses?myid="+id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                    lc = JsonConvert.DeserializeObject<List<Course>>(apiResponse);
                }
